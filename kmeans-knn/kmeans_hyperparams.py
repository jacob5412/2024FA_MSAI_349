...
logger = logging.getLogger("kmeans-hyperparameter-selection")
...
def read_hyperparams(filepath):
    with open(filepath, "r", newline="") as csvfile:
        ...
        for row in reader:
            k_components = int(row["k_components"])
            pca_components = (
            ...
            yield (k_components, pca_components, scaler, distance_metric)
...
def get_best_hyperparams(
    ...
    for hyperparams in read_hyperparams(hyperparam_filepath):
        ...
        for _ in range(num_iterations):
            ...
            # Train Kmeans
            kmeans = KMeans(k_component)
            kmeans.fit(transformed_train_features, distance_metric)
            predicted_labels = kmeans.predict(
                transformed_valid_features,
                shuffled_validation_set_labels,
                distance_metric,
            )
        ...
        csv_data.append(
            [k_component, pca_component, scaler, distance_metric, avg_accuracy]
        )

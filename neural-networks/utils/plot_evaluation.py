...
def plot_learning_curve(train_losses, valid_losses, hyperparams, base_path=None):
    ...
    hyperparams_str = (
        ...
        + f"{hyperparams[2]}, LR Decay Step: {hyperparams[3]}"
    )
    plt.text(
        0.5,
        0.95,
    ...
    plt.savefig(
        f"{base_path}/lc_{hyperparams[0]}_{hyperparams[1]}_{hyperparams[2]}_{hyperparams[3]}.png"
    )
    plt.close()
...
def plot_accuracy_curve(train_accuracies, valid_accuracies, hyperparams, base_path=None):
    ...
    plt.savefig(
        f"{base_path}/acc_{hyperparams[0]}_{hyperparams[1]}_{hyperparams[2]}_{hyperparams[3]}.png"
    )
    plt.close()
